...
```bash
iiverilog -o output/pre_synth_sim/pre_synth_sim.out -DPRE_SYNTH_SIM -I src/include -I src/module src/module/testbench.v
```
...
## Running Pre-Synthesis Simulation and Viewing Waveforms 📊

To verify the design functionality before synthesis, we perform a **pre-synthesis simulation** using `iverilog`.
The simulation generates a **VCD (Value Change Dump) file**, which stores signal transitions.
This file can then be visualized with **GTKWave** to analyze the behavior of the circuit.
...
At this stage, the simulation will produce a `pre_synth_sim.vcd` file inside the same directory.
This file contains all the waveform information captured during the testbench run.
...
Now we are back inside the **vsdbabysoc** root folder.
...
* **`pre_synth_sim.out`** → Compiled simulation executable generated by `iverilog`. Running this file executes the testbench.
* **`pre_synth_sim.vcd`** → Waveform dump file automatically created during simulation.
* **`gtkwave`** → Tool used to visualize the signals and verify correct functionality of the design.
